...
export const useImageUploadWithCaption = ({ projectId, onImageAdd }: UseImageUploadWithCaptionProps) => {
  ...
  const handleImageAdd = useCallback(async () => {
    ...
    try {
      ...
      const caption = await generateCaptionForNewImage(selectedImageSrc);
      
      if (caption) {
        toast.success('AI caption generated and saved!');
      } else {
        toast.error('Failed to generate caption, but image added successfully');
      }
      
      console.log('✅ Image upload with caption complete');
  ...
  }, [onImageAdd, isSelecting, projectId, generateCaptionForNewImage]);

  return {
    isSelecting: isSelecting || isGenerating,
    handleImageAdd
  };
};
