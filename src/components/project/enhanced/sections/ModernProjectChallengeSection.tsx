          ...
          {details.challengeGalleryImages.map((image, index) => (
            <React.Fragment key={index}>
              ...
              {index === 0 && isInvestorProject && (
                <div className="p-6 bg-blue-50/50 rounded-lg border border-blue-100">
                  <EnhancedContentEditor
                    ...
                    contentType="header"
                    onSave={(content) => handleSectionContentSave('challenge', 'text', content, 'research_discovery_title')}
                    className="mb-4"
                    projectId={projectId}
                  />
                  ...
                  <div className="space-y-6">
                    <EnhancedContentEditor
                      content="I conducted extensive research with the banking team to understand their current pain points, identifying critical issues including manual data entry errors causing compliance issues, time-consuming Excel-based tracking processes, difficulty collaborating on complex deals, limited search and filtering capabilities, and email-based communication creating information silos. Through this research, I developed three key user personas: Investment Managers who need quick access to deal information and real-time updates, Loan Officers who require efficient deal processing and client communication tools, and Compliance Officers who need accurate reporting and audit trails."
                      contentType="section"
                      onSave={(content) => handleSectionContentSave('challenge', 'text', content, 'research_discovery_paragraph1')}
                      className="mb-4"
                      projectId={projectId}
                    />
                    
                    <EnhancedContentEditor
                      content="My competitive analysis focused on existing financial platforms, with particular emphasis on Bloomberg's search functionality, which became a key inspiration for our AI-powered search feature. This analysis revealed opportunities to modernize traditional banking workflows while maintaining the professional standards and regulatory compliance requirements that are essential in the financial services industry."
                      contentType="section"
                      onSave={(content) => handleSectionContentSave('challenge', 'text', content, 'research_discovery_paragraph2')}
                      projectId={projectId}
                    />
                  </div>
                </div>
              ...
              {index === 0 && !isInvestorProject && details.challengeAdditionalText && (
                <div className="p-6 bg-blue-50/50 rounded-lg border border-blue-100">
                  <EnhancedContentEditor
                    content={details.challengeAdditionalText}
                    contentType="section"
                    onSave={(content) => handleSectionContentSave('challenge', 'text', content, 'challenge_additional_text')}
                    projectId={projectId}
                  />
                </div>
              )}
