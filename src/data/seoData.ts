...
// changed its visible copy (style-only commits do not count). Not invented, and
// not a blanket "today": the Article schema was scoring 67% completeness purely
// for missing them. These are hand-maintained and drift: on 2026-09-13 every
// study still said 2026-08-29 after all thirteen were rewritten on 09-12. Run
// `node scripts/check-content-dates.mjs` to see where git is newer than this.
...
//
// `modified` follows the same rule as PROJECT_SEO_MAP: the last commit that
// changed the post's visible copy, per post (`git log -L` on its block), not the
// file's last commit and not a class tweak. Feeds BlogPosting.dateModified and
