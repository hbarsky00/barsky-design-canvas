...
- **2026-09-15 — the FAQ cards disappeared when a reader scrolled back to
  them.** Design was the staler half (design log last 09-11; the AEO half
  ran 09-13 on content dates, closing cycle 1 of the six-lever rotation).

  **Diagnosed first, across the whole site, before picking anything.** Two
  stale `vite preview` processes were killed at start (one from this
  morning on 4173, one orphaned since 09-13 on 4199); no concurrent writer
  — the tree was clean and the last commit was nine hours old, though the
  09-14/09-15 history shows another session rewrote every case study's copy
  and re-shot most heroes yesterday, so the case studies got looked at
  hardest. Measured on the built site, headless Chrome over CDP:
  - **Layout shift, 21 non-blog routes × 375/1440 = 42 combos: CLS 0.0000
    on 37, 0.0006 on 5** (one 1px shift each, all at 1440). Every image
    and every `<video>` that affects layout carries dimensions. The
    observer was validated first by injecting a 300px block and reading
    back 0.369 — a checker that cannot fail proves nothing.
  - `check:contrast --all`: unchanged from the 09-07 baseline.
  - `seo_audit_all_routes.py` PROBLEMS: 0. `check-content-dates.mjs`:
    **16 stale**, see below.
  - Viewport-by-viewport frames of `/` and six case studies at both widths,
    reviewed as contact sheets (full-page capture was abandoned: setting the
    viewport to page height blew every `min-h-screen` section up to 15,000px,
    the exact trap `contrast-walk`'s header documents). Nothing clipped,
    nothing overflowed (`scrollWidth` 375 = `innerWidth` on `/` and
    `/project/ring-rival`), 0 images without dimensions, 0 tap targets
    under 24px on either.

  What the frames did show was the FAQ at the bottom of `/`: whole answers
  rendered blank mid-scroll while their neighbours were visible. Chased
  down rather than assumed — it was not capture timing.

  **The defect.** `SeoFaqSection.tsx` was the only `whileInView` on the
  site without `viewport={{ once: true }}` — 52 other call sites have it.
  Without `once`, framer-motion animates a card back to its `initial`
  (opacity 0, y 20) when it leaves the viewport, and then replays the
  `delay: index * 0.1` stagger on re-entry. A stagger is for cards that
  appear together; on this section one card fills a phone viewport
  (heights 485–624px at 375), so the delay just holds a lone card blank.

  Measured on the built site, reading pace (300px every 250ms) to the end of
  the FAQ, then back up one answer — what someone re-reading does:

  | | 375 | 1440 |
  |---|---|---|
  | answer's opacity on return, before | **0 for ~800ms**, then 0.6s fade | **0 for ~700ms**, then 0.6s fade |
  | time to legible, before | **~1.4s** | **~1.3s** |
  | min opacity while in view, after | **1.00** | **1.00** |

  A cruder first probe (jump to the last card, jump away, jump back) caught
  the mechanism outright: on return the card faded **1 → 0** over 0.6s
  (the delayed exit landing after re-entry), sat at 0 until 1.7s, and was
  back at 1 at 2.4s. Text a reader had already seen once, vanishing in
  front of them.

  **Changed — `src/components/seo/SeoFaqSection.tsx`, two props.**
  `viewport={{ once: true }}` on the heading wrapper and on each card, with
  a comment carrying the measurement so nobody removes it for "livelier"
  motion. Duration, stagger, easing, markup, copy: untouched. First
  appearance fades in exactly as before and exactly as every other section.

  **Rider, from the mandated date check — `src/data/seoData.ts`,
  `scripts/check-content-dates.mjs`.** 12 case studies declared
  `modified: 2026-09-13`; git says 09-14 (the "replace every line of copy
  with Hiram's own rewrite" commits) and 09-15 for six of them (new heroes
  with new alts: e.g. Recast `card-product.webp` → `card-footage.webp`,
  1600×1000 → 1004×628). Bumped to git's dates by the playbook's rule. The
  four blog posts the check flagged were touched only by `62ad56ba`, which
  unlinked four Investor Loan sentences without changing a word — added to
  `STYLE_ONLY` rather than bumping four `dateModified`s for nothing.
  `check-content-dates.mjs` now reports **35 routes, 0 stale** (35, not 36:
  Investor Loan is off the site since 09-14). `public/sitemap.xml`
  regenerated by prebuild, 12 `<lastmod>` lines changed, committed with it.

  **Measured.** `tsc --noEmit` 0, `eslint` 0 on the three files, build
  **44 / 44 prerendered, 0 head-only**. No `capture-bodies`: a Motion prop
  and head dates, no visible copy. Built `dist/project/recast/index.html`
  carries `dateModified: 2026-09-15` and the sitemap agrees. Staged by
  explicit path; the tree was clean before and after.

  **Verified on barskydesign.pro, not on the local build.** Live ~90s after
  the push (`5da5ca1f`): the served chunk is `index-DPn0Oi07.js`, the same
  hash as `dist/`, and the compiled FAQ section carries `once:!0`. The
  re-read measurement re-run against production: **min opacity while in
  view 1.00 at 375 and 1440**, 30 samples each, no dip. `sitemap.xml` live
  serves 6 × `2026-09-14` and 6 × `2026-09-15`; `/project/recast` serves
  `dateModified: 2026-09-15` and `/project/ring-rival` `2026-09-14`,
  matching git and matching the sitemap.

  **Left open:**
  - **"Hover to watch it run"** — the label on every case-study hero with a
    walkthrough clip (12 pages, `CaseStudyHeroMedia`) names an interaction
    phones do not have. Tapping does work (`onClick` toggles), so it is the
    copy that is wrong, not the control; a `[@media(hover:none)]` variant
    swapping in "Tap to watch it run" is the fix. Not a rider on a motion
    fix — it is copy on the pages to touch least casually, and it would
    have meant a `capture-bodies` run on 12 routes.
  - The homepage contact section is `min-h-screen flex justify-center` at
    1440×900, so ~220px of empty band sits above and below a ~450px form.
    Reads as airy rather than broken, and Hiram likes full-viewport
    sections; noted, not changed.
  - In headless Chrome every FAQ card reports opacity 1 before any
    scrolling, which means `whileInView` fired for all of them at mount.
    Not reproduced in a real viewport and not caused by this change (the
    pre-fix probe showed the same), but a future run measuring first-
    appearance timing should know the headless number is not the reader's.
  - Unchanged from 09-11: the `contrast-walk` opacity blind spot (fully
    diagnosed 09-07), `heading-order` on `/blog` and `/store`, the
    case-study tag separator at 1.37:1, comments still off (two env vars,
    Hiram's call), `<meta name="theme-color">` still the dead blue
    `#3B82F6`.
